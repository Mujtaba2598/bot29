...
// FIXED Binance API with Time Sync and Rate Limit Protection
class BinanceAPI {
    ...
    // CRITICAL FIX: Get server time to avoid timestamp errors
    ...
    static validateApiKey(apiKey) {
        // Binance API keys are usually 64+ characters
    ...
    static async makeRequest(endpoint, method, apiKey, secret, params = {}, useTestnet = false) {
        try {
            ...
            // CRITICAL FIX: Use server time instead of local time
            ...
            console.log(`📡 Request to ${endpoint} with timestamp ${timestamp}`);
            
            const response = await axios({
                method,
                url,
                headers: { 'X-MBX-APIKEY': apiKey.trim() }, // Trim any whitespace
                timeout: 10000
            });
            ...
            if (error.response) {
                ...
                // Handle timestamp error (MOST COMMON)
                if (data.code === -1021) {
                    console.log('⏰ TIMESTAMP ERROR: Server time out of sync');
                    console.log('   Local time:', new Date().toISOString());
                    console.log('   Attempting to sync...');
                    
                    // Force a server time sync on next request
...
app.listen(PORT, '0.0.0.0', () => {
    console.log('\n' + '='.repeat(60));
    console.log('🌙 HALAL AI TRADING BOT - TIME SYNC FIXED VERSION');
    console.log('='.repeat(60));
    console.log(`✅ Server running on port: ${PORT}`);
    console.log(`✅ Time sync: Using Binance server time`);
    console.log(`✅ Key validation: Automatic format checking`);
